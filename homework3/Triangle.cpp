...
//array<Vector4f, 3>表示一个数组中，可以放3个vector4f;1个vector4f表示一个顶点的(x,y,z,w)
std::array<Vector4f, 3> Triangle::toVector4() const
{
    std::array<Vector4f, 3> res;
	//transform:可以一元运算/二元运算，这里是一元；
	//参数：输入的起始iterator，输入的结束iterator，结果的iterator，一元运算规则
	//这里[](auto& vec) { return Vector4f(vec.x(), vec.y(), vec.z(), 1.f); }
	//是lambda表达式，表示返回(x,y,z,1)
